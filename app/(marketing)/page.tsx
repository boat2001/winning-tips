...
import { ChevronRight, Crown, MessageCircle, Send, Trophy, Users, Zap } from "lucide-react";
...
export default async function LandingPage(){
  ...
  const [data,board]=await Promise.all([getTipsData(),getFreeTipsBoard()]);
  return <><section className="landing-hero">
    ...
    <div className="landing-copy"><p className="landing-kicker">Africa&apos;s sports prediction community</p><h1>Your Home for<br/><span>Smart Sports</span><br/>Predictions</h1>
      ...
      <div className="landing-actions"><Link href="/tips#vip-slips" className="landing-join"><Crown aria-hidden/><span>Check VIP Tips<small>TODAY&apos;S PREMIUM SLIPS</small></span><ChevronRight aria-hidden/></Link><Link href="#free-tips" className="landing-picks">Free Tips <ChevronRight aria-hidden/></Link></div>
      <div className="landing-channels"><span>Join our free community</span><a href={communityLinks.telegram} target="_blank" rel="noreferrer"><Send aria-hidden/>Telegram<span className="sr-only"> (opens in a new tab)</span></a><a href={communityLinks.whatsapp} target="_blank" rel="noreferrer"><MessageCircle aria-hidden/>WhatsApp<span className="sr-only"> (opens in a new tab)</span></a></div>
  ...
  </section><LandingSections tips={data.tips} board={board} timezone={launchCountry.timezone}/><SiteLinksFooter/></>;
}

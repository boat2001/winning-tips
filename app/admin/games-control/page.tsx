...
export default async function GamesControlPage({ searchParams }: { searchParams?: Promise<{ applied?: string; country?: string }> }) {
  ...
  const [plans, bookings] = await Promise.all([
    ...
    database.booking.findMany({ where: { countryCode: country.countryCode, bookingDate: { gte: start, lt: end }, isActive: true, category: { in: ["VIP1", "VIP2", "VIP3"] } }, select: { category: true }, orderBy: { createdAt: "desc" } }),
  ]);
            ...
            <article key={plan.id} className="rounded-sharp border border-line bg-white p-5 text-center">
              ...
              <form action={updateVipControl} className="mt-5">
                ...
                <div className="mt-2 flex gap-2">
                  <input name="price" type="number" min="0.01" step="0.01" required defaultValue={plan.priceMinor > 0 ? String(plan.priceMinor / 100) : "0"} aria-label={`${plan.name} price in ${country.currency}`} className="h-11 min-w-0 flex-1 rounded-sharp border border-line px-3 text-center text-sm font-bold outline-none focus:border-blue" />
